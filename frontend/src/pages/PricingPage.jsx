...
const PricingPage = () => {
  ...
  const handleGetStarted = (planId) => {
    console.log('🔍 Pricing button clicked:', { isAuthenticated, user: user?.email });
    
    if (isAuthenticated && user) {
      // User is logged in, go to payment/checkout
      console.log('✅ User logged in, redirecting to checkout');
      navigate(`/checkout?plan=${planId}`);
    } else {
      // User not logged in, go to register
      console.log('⚠️ User not logged in, redirecting to register');
      navigate('/register');
    }
  };
              ...
              <motion.div key={plan.name} variants={staggerItem}>
                ...
                >
                  ...
                  <CardFooter>
                    ...
                    >
                      {isAuthenticated && user ? 'Upgrade Now' : 'Get Started'}
                    </AnimatedButton>
                  </CardFooter>
                </Card>
              </motion.div>
