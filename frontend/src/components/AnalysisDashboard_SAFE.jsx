...
export function AnalysisDashboard({ analysis }) {
  ...
  const VerdictBadge = () => {
    ...
    const verdict = safeString(analysis.ai_verdict, 'Unknown').split(' ')[0];
    return (
      <Badge className={`${colors[verdict] || colors.Medium} border`}>
        {safeString(analysis.ai_verdict, 'Unknown')}
      </Badge>
    );
  };
    ...
    <div className="space-y-6" data-testid="analysis-dashboard">
        ...
        <div className="bg-gradient-to-r from-purple-900/30 to-pink-900/30 border border-purple-500/30 rounded-lg p-6">
          <div className="flex flex-col md:flex-row gap-6 justify-between items-start">
            <div className="flex-1">
              ...
              <p className="text-slate-300 text-lg leading-relaxed">{safeString(analysis.overview, 'No overview available')}</p>
            ...
            <div className="min-w-[200px] bg-slate-950/50 p-4 rounded-xl border border-white/10 text-center">
              ...
              <div className="text-4xl font-bold text-white mb-2">{analysis.confidence_score || 0}%</div>
              <Progress value={analysis.confidence_score || 0} className="h-1.5 bg-slate-800" indicatorClassName="bg-gradient-to-r from-green-400 to-emerald-500" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 pt-6 border-t border-white/5">
            ...
            <div>
              ...
              <div className="text-xl font-bold text-purple-400">{analysis.virality_score || 0}/100</div>
            </div>
            <div>
              <div className="text-slate-400 text-xs mb-1">Retention Score</div>
              <div className="text-xl font-bold text-pink-400">{analysis.retention_score || 0}/100</div>
            </div>
            <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="bg-slate-900/50 border-slate-800 col-span-1 lg:col-span-1" data-testid="ai-insights-card">
            ...
            <CardContent className="space-y-3">
              {safeArray(analysis.ai_insights).map((insight, idx) => (
                <div key={idx} className="bg-slate-800/30 rounded-lg p-3 border border-slate-700/50">
                  <div className="flex items-start justify-between mb-1">
                    <Badge variant="outline" className="text-purple-400 border-purple-400/30 text-[10px]">
                      {safeString(insight?.insight_type, 'Insight')}
                    </Badge>
                    <span className="text-xs text-slate-500">{insight?.confidence || 0}% Conf.</span>
                  </div>
                  <p className="text-slate-300 text-sm">{safeString(insight?.description, '')}</p>
                </div>
              ))}
            </CardContent>
          </Card>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <Card className="bg-slate-900/50 border-slate-800" data-testid="market-analysis-card">
              ...
              <CardContent>
                <SWOTAnalysisGrid marketAnalysis={safeObject(analysis.market_analysis)} />
          ...
          <div className="space-y-6">
            <Card className="bg-slate-900/50 border-slate-800" data-testid="user-personas-card">
              ...
              <CardContent className="space-y-4">
                {safeArray(analysis.user_personas).slice(0, 2).map((persona, idx) => (
                  <div key={idx} className="bg-slate-800/30 p-3 rounded-lg border border-slate-700/50">
                    <div className="font-semibold text-white text-sm mb-1">{safeString(persona?.name, 'Persona')}</div>
                    <div className="text-xs text-slate-400 italic mb-2">{safeString(persona?.demographics, '')}</div>
                    <div className="flex flex-wrap gap-1">
                      {safeArray(persona?.pain_points).slice(0, 2).map((p, i) => (
                        <Badge key={i} variant="secondary" className="bg-slate-800 text-[10px] text-slate-300">{safeString(p)}</Badge>
                      ))}
                    </div>
                  </div>
                ))}
              ...
              <CardContent>
                <div className="text-center py-2">
                  ...
                  <div className="text-2xl font-bold text-white mt-1">
                    {safeString(analysis.revenue_projection?.min_monthly, '$0')} - {safeString(analysis.revenue_projection?.max_monthly, '$0')}
                  </div>
                  <div className="text-xs text-green-400 mt-1">Timeline: {safeString(analysis.revenue_projection?.growth_timeline, 'TBD')}</div>
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card className="bg-gradient-to-br from-green-900/30 to-emerald-900/30 border-green-500/30">
            ...
            <CardContent>
              <p className="text-white text-sm leading-relaxed">{safeString(analysis.biggest_opportunity, 'Not specified')}</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-yellow-900/30 to-orange-900/30 border-yellow-500/30">
            ...
            <CardContent>
              <p className="text-white text-sm leading-relaxed">{safeString(analysis.biggest_risk, 'Not specified')}</p>
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      <div className="space-y-4">
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col md:flex-row gap-6 items-center">
              <div className="flex-1">
                ...
                <h3 className="text-2xl font-bold text-white mb-2">{safeString(analysis.next_action, 'Not specified')}</h3>
        ...
        <Card className="bg-slate-900/50 border-slate-800" data-testid="strategies-card">
          ...
          <CardContent>
            {safeArray(analysis.strategies).length > 0 ? (
              <Tabs defaultValue={safeArray(analysis.strategies)[0]?.channel || 'SEO'} className="w-full">
                <TabsList className="grid w-full grid-cols-4 bg-slate-800">
                  {safeArray(analysis.strategies).map((strategy, idx) => (
                    <TabsTrigger
                      key={idx}
                      value={safeString(strategy?.channel, `Channel ${idx + 1}`)}
                      data-testid={`strategy-tab-${safeString(strategy?.channel, '').toLowerCase().replace(/\s+/g, '-')}`}
                      className="data-[state=active]:bg-purple-600"
                    >
                      {safeString(strategy?.channel, `Channel ${idx + 1}`)}
                    </TabsTrigger>
                  ))}
                </TabsList>
                {safeArray(analysis.strategies).map((strategy, idx) => (
                  <TabsContent key={idx} value={safeString(strategy?.channel, `Channel ${idx + 1}`)} className="space-y-4 mt-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {/* Left Col: Strategy Details */}
                      <div className="space-y-4">
                        <div>
                          <h4 className="text-white font-semibold mb-2">Tactical Approach</h4>
                          <p className="text-slate-300 text-sm leading-relaxed">{safeString(strategy?.strategy, 'No strategy details available')}</p>
                        </div>
                        <div>
                          <h4 className="text-white font-semibold mb-2">Content Angles</h4>
                          <ul className="space-y-2">
                            {safeArray(strategy?.content_ideas).length > 0 ? (
                              safeArray(strategy?.content_ideas).map((idea, i) => (
                                <li key={i} className="flex items-start gap-2 text-slate-300 text-sm">
                                  <ArrowRight className="w-4 h-4 text-purple-400 mt-0.5 flex-shrink-0" />
                                  <span>{safeString(idea)}</span>
                                </li>
                              ))
                            ) : (
                        ...
                        <div>
                          ...
                          <div className="space-y-1">
                            {Object.keys(safeObject(strategy?.kpi_benchmarks)).length > 0 ? (
                              Object.entries(safeObject(strategy?.kpi_benchmarks)).map(([key, value], i) => (
                                <div key={i} className="flex justify-between text-sm">
                                  <span className="text-slate-400">{safeString(key).replace(/_/g, ' ')}</span>
                                  <span className="text-white font-mono">{safeString(value)}</span>
                                </div>
                              ))
                            ) : (
                              <div className="text-slate-400 text-sm">No KPI benchmarks available</div>
                            )}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-950/30 rounded-xl p-4 border border-slate-800/50">
                        ...
                        <AdPreviewCard
                          platform={safeString(strategy?.channel, 'SEO')}
                          content={safeString(safeArray(strategy?.content_ideas)[0], 'Sample content')}
                          headline={safeString(safeArray(strategy?.content_ideas)[1], 'AstraMark Strategy')}
                        />
                      </div>
                    </div>
                  </TabsContent>
              ...
              <div className="text-slate-400 text-center py-8">No strategies available</div>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <div className="space-y-4">
        ...
        <ExecutionActions actions={safeArray(analysis.execution_actions)} isPremium={isPremium} />
        <ContentActionsPanel analysisId={safeString(analysis.id, 'unknown')} isPremium={isPremium} />
      </div>
