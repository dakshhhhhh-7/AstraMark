...
export function AnalysisDashboard({ analysis }) {
  ...
  const safeAnalysis = {
    ...
    ai_verdict: analysis.ai_verdict || 'Unknown',
    virality_score: analysis.virality_score || 0,
    retention_score: analysis.retention_score || 0,
    ai_insights: Array.isArray(analysis.ai_insights) ? analysis.ai_insights : [],
    user_personas: Array.isArray(analysis.user_personas) ? analysis.user_personas : [],
    strategies: Array.isArray(analysis.strategies) ? analysis.strategies : [],
    market_analysis: analysis.market_analysis || {},
    revenue_projection: analysis.revenue_projection || {},
    biggest_opportunity: analysis.biggest_opportunity || 'Not specified',
    biggest_risk: analysis.biggest_risk || 'Not specified',
    next_action: analysis.next_action || 'Not specified',
    execution_actions: Array.isArray(analysis.execution_actions) ? analysis.execution_actions : [],
    id: analysis.id || 'unknown',
    blockchain_proof: analysis.blockchain_proof || null,
    market_signals: Array.isArray(analysis.market_signals) ? analysis.market_signals : [],
    ai_learning_updates: Array.isArray(analysis.ai_learning_updates) ? analysis.ai_learning_updates : []
  };
  ...
  const VerdictBadge = () => {
    ...
    const verdict = String(safeAnalysis.ai_verdict || '').split(' ')[0];
    return (
      <Badge className={`${colors[verdict] || colors.Medium} border`}>
        {safeAnalysis.ai_verdict}
      </Badge>
    );
  };
        ...
        <div className="bg-gradient-to-r from-purple-900/30 to-pink-900/30 border border-purple-500/30 rounded-lg p-6">
          <div className="flex flex-col md:flex-row gap-6 justify-between items-start">
            <div className="flex-1">
              ...
              <p className="text-slate-300 text-lg leading-relaxed">{safeAnalysis.overview}</p>
            ...
            <div className="min-w-[200px] bg-slate-950/50 p-4 rounded-xl border border-white/10 text-center">
              ...
              <div className="text-4xl font-bold text-white mb-2">{safeAnalysis.confidence_score}%</div>
              <Progress value={safeAnalysis.confidence_score} className="h-1.5 bg-slate-800" indicatorClassName="bg-gradient-to-r from-green-400 to-emerald-500" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 pt-6 border-t border-white/5">
            ...
            <div>
              ...
              <div className="text-xl font-bold text-purple-400">{safeAnalysis.virality_score}/100</div>
            </div>
            <div>
              <div className="text-slate-400 text-xs mb-1">Retention Score</div>
              <div className="text-xl font-bold text-pink-400">{safeAnalysis.retention_score}/100</div>
            </div>
            <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="bg-slate-900/50 border-slate-800 col-span-1 lg:col-span-1" data-testid="ai-insights-card">
            ...
            <CardContent className="space-y-3">
              {safeAnalysis.ai_insights.map((insight, idx) => (
                <div key={idx} className="bg-slate-800/30 rounded-lg p-3 border border-slate-700/50">
                  <div className="flex items-start justify-between mb-1">
                    <Badge variant="outline" className="text-purple-400 border-purple-400/30 text-[10px]">
                      {String(insight?.insight_type || 'Insight')}
                    </Badge>
                    <span className="text-xs text-slate-500">{insight?.confidence || 0}% Conf.</span>
                  </div>
                  <p className="text-slate-300 text-sm">{String(insight?.description || '')}</p>
                </div>
              ))}
            </CardContent>
          </Card>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="space-y-1">
                          {strategy.kpi_benchmarks && typeof strategy.kpi_benchmarks === 'object' && Object.entries(strategy.kpi_benchmarks).map(([key, value], i) => (
                            <div key={i} className="flex justify-between text-sm">
                              <span className="text-slate-400">{key.replace(/_/g, ' ')}</span>
                              <span className="text-white font-mono">{typeof value === 'string' || typeof value === 'number' ? value : JSON.stringify(value)}</span>
                            </div>
                          ))}
                    ...
                    <div className="bg-slate-950/30 rounded-xl p-4 border border-slate-800/50">
                      ...
                      <AdPreviewCard
                        ...
                        content={strategy.content_ideas && Array.isArray(strategy.content_ideas) && strategy.content_ideas[0] ? (typeof strategy.content_ideas[0] === 'string' ? strategy.content_ideas[0] : JSON.stringify(strategy.content_ideas[0])) : "Sample content"}
                        headline={strategy.content_ideas && Array.isArray(strategy.content_ideas) && strategy.content_ideas[1] ? (typeof strategy.content_ideas[1] === 'string' ? strategy.content_ideas[1] : JSON.stringify(strategy.content_ideas[1])) : "AstraMark Strategy"}
                      />
                    </div>
                  </div>
                </TabsContent>
